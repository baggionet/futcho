...
const agregarUsuario = async (req, res) => {
    try {
        ...
        const crearCuenta = await models.cuenta.create(
            {
                saldo: bodySaldo,
                saldoAnterior: bodySaldoAnterior
            }
        );
        ...
        console.log(body);
        const creandoUsuario = await models.usuario.create({
            nombre: body.nombre,
            correo: body.correo,
            usuario: body.usuario,
            contraseña: body.contraseña,
            foto: userfoto,
            cuentaId: crearCuenta.id,
            rollId: body.rollId
        });
...
const listarUsuarios = async (req, res) => {
    try {
        const listar = await models.usuario.findAll({
            attributes: ['nombre', 'correo', 'usuario', 'foto', 'cuentaId', 'rollId']
        });
...
const usuarioXId = async (req, res) => {
    try {
        ...
        const buscarId = await models.usuario.findByPk(pk,{
            attributes: ['nombre', 'correo', 'usuario', 'foto', 'cuentaId', 'rollId'],
            include: {
                model: models.cuenta,
                attributes:['id', 'saldo', 'saldoAnterior']
            }
        });
...
const borrarUsuario = async ( req, res ) => {
    try {
        ...
        const buscarUsuario = await models.usuarios.sindByPk(pk);
        if(isEmpty(buscarUsuario)) return res.status(404).send('Usuario no encontrado');

        const removerUsuario = await models.usuario.destroy({
...
export { agregarUsuario, listarUsuarios, usuarioXId, borrarUsuario };