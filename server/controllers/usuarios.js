import models from '../../models';
import { isEmpty } from '../utils/methods';
import { fileUpload } from '../utils/uploadImages';
import jwt from 'jsonwebtoken';
import bcrypt from 'bcryptjs';
import { SEED } from '../../config/keys';


/*
*Enpoint para agregar usuario
*Datos requeridos
*
* nombre
* correo
* usuario
* contraseña
* foto
* rollId
*
*
*/
const agregarUsuario = async (req, res) => {
    try {
        const { body } = req;
        let userfoto = null;
        const saltRounds = 10;
//Variable para la cuenta 
        let bodySaldo = 0;
        let bodySaldoAnterior = 0;
// se valida los campos 
        if (isEmpty(body.name)) return res.status(400).send('El nombre es requerido');
        if (isEmpty(body.email)) return res.status(400).send('El correo es requerido');
        if (isEmpty(body.nickName)) return res.status(400).send('El nombre de usuario es requerido');
        if (isEmpty(body.password)) return res.status(400).send('La contraseña es requerida');
//Se crea la cuenta primero que se asociara al usuario.
        const crearCuenta = await models.account.create(
            {
                balance: bodySaldo,
                previousBalance: bodySaldoAnterior
            }
        );

        const pass = await bcrypt.hash(body.password, saltRounds );
        
        const imageDefaul = "data:image/jpeg;base64,";
        if (isEmpty(body.photo)){
            userfoto = await fileUpload(imageDefaul, 'perfiles');
        }else{
            userfoto = await fileUpload(body.foto, 'perfiles');
        }
        

        
        const creandoUsuario = await models.users.create({
            name: body.name,
            email: body.email,
            nickName: body.nickName,
            password: pass,
            photo: userfoto,
            accountId: crearCuenta.id,
            rollId: body.rollId
        });
        return res.status(201).send(creandoUsuario);
    } catch (error) {
        res.status(500).send(error.message);
    }
};

/*
* Listara todos los usuarios
*
*/
const listarUsuarios = async (req, res) => {
    try {
        const listar = await models.users.findAll({
            attributes: ['name', 'email', 'nickName', 'photo', 'accountId', 'rollId']
        });

        return res.status(201).send(listar);
    } catch (error) {
        res.status(500).send(error.message);
    }
};
/*
*Buscar usuario por Id
*Dato requerido
* id
*/
const usuarioXId = async (req, res) => {
    try {
        const pk = req.params.usuarioId;

        const buscarId = await models.users.findByPk(pk,{
            attributes: ['name', 'user', 'nickName', 'photo', 'accountId', 'rollId'],
            include: {
                model: models.account,
                attributes:['id', 'balance', 'previousBalance']
            }
        });
        if (isEmpty(buscarId)) return res.status(204).send('Usuario no encontrado');
        
        return res.status(201).send(buscarId);
    } catch (error) {
        res.status(500).send(error.message);
    }
};
/*
*
*Se borrara usuario buscado por Id
* Dato requerido
* id
*
*/
const borrarUsuario = async ( req, res ) => {
    try {
        const pk = req.params.usuarioId;
        const { body } = req;
        body.statusDelete = true;
        const buscarUsuario = await models.users.findByPk(pk);
        if(isEmpty(buscarUsuario)) return res.status(404).send('Usuario no encontrado');

    } catch (error) {
        res.status(500).send(error.message);
    } 
};

/*
*
* Login de usuario
* Dato requerido
* correo
* contraseña
*/
const login = async (req, res) =>{
    try {
        const { body } = req

        if(isEmpty(body.email)) return res.status(404).send("El correo es requerido")
        if(isEmpty(body.password)) return res.status(404).send("La contraseña es requerida")

        const busquedaUser = await models.users.findOne({
            where: {email: body.email},
            
        })
        console.log(busquedaUser)
        if(!busquedaUser) return res.status(401).send('El correo es invalido');
        if(!bcrypt.compareSync(body.password, busquedaUser.password)) return res.status(401).send('Contraseña incorrecta');


        const response = await models.users.findByPk(busquedaUser.id,{
            attributes: ['id', 'name', 'email', 'nickName', 'photo', 'accountId', 'rollId'],
            include:{
                model: models.account,
                attributes: ['id', 'balance', 'previousBalance'],
            },
        });
        console.log("imprimimos el response", response);

        const token = jwt.sign({user: response}, SEED, {expiresIn: 60 * 60});
        console.log(token)
        if (!token) return res.status(500).send('Hubo un problema intenta de nuevo');



        return res.status(200).send({token, response});


    } catch (error) {
        console.log(error);
        return res.status(500).send(error.message);
    }
}

export { agregarUsuario, listarUsuarios, usuarioXId, borrarUsuario, login };